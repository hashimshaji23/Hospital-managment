...
export async function createDoctor(req, res) {
  ...
  try {
    ...
    if (req.file?.path) {
      const uploded = await uploadToCloudinary(req.file.path, "medicare");
      imageUrl = uploded?.secure_url || uploded?.url || imageUrl;
      imagePublicId = uploded?.public_id || uploded?.public_id || imagePublicId;
    }
...
export async function toggleAvilability(req, res) {
  ...
  try {
    ...
    console.error("togleAvailabilty error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
