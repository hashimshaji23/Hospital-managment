...
const ServiceAppointments = () => {
    ...
    const load = async () => {
        ...
        try {
            ...
            const res = await api.get(`/service-appointments${params.toString() ? `?${params}` : ""}`)
            setAppointments(res.appointments || [])
        } catch (err) {
            setError(err.message || "Could not load service appointments")
        } finally {
            setLoading(false)
        }
    }
    ...
    const updateStatus = async (id, status) => {
        setUpdatingId(id)
        try {
            await api.put(`/service-appointments/${id}`, { status })
            load()
        } catch (err) {
    ...
    const cancelAppointment = async (id) => {
        setUpdatingId(id)
        try {
            await api.post(`/service-appointments/${id}/cancel`, {})
            load()
        } catch (err) {
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <h1 className="text-2xl font-extrabold text-emerald-900">Service Appointments</h1>
            ...
            <div className="bg-white rounded-2xl border border-emerald-100 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <p className="text-center text-gray-500 py-10">No service appointments found.</p>
                ) : (
                            ...
                            <tbody>
                                {appointments.map((a) => (
                                    <tr key={a._id} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                                        ...
                                        <td className="py-3 px-4 text-gray-600">{a.serviceName}</td>
                                        <td className="py-3 px-4 text-gray-600">
                                            {a.date} · {a.hour}:{String(a.minute).padStart(2, "0")} {a.ampm}
                                        </td>
